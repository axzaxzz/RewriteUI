...
function library:Init(key)
    ...
    -- MATCHA: Drop shadow (3D floating effect)
    ...
    shadowContainer.Size = UDim2.new(0, 490, 0, 740)  -- 40px larger for shadow
    shadowContainer.Image = "rbxasset://textures/ui/GuiImagePlaceholder.png"  -- Will be black
    shadowContainer.ImageColor3 = Color3.fromRGB(0, 0, 0)
    shadowContainer.ImageTransparency = 0.5  -- 50% opacity shadow
    shadowContainer.ScaleType = Enum.ScaleType.Slice
    shadowContainer.SliceCenter = Rect.new(10, 10, 10, 10)
    ...
    mainWindow.Size = UDim2.new(0, 450, 0, 700)  -- NARROW & TALL!
    library:TrackElement(mainWindow, "edgeDark")
    library.mainEdge = mainWindow  -- For compatibility
    ...
    closeButton.MouseButton1Click:Connect(function()
        -- Smooth fade out
        TweenService:Create(mainWindow, TweenInfo.new(0.2, Enum.EasingStyle.Cubic), {Size = UDim2.new(0, 0, 0, 0)}):Play()
        TweenService:Create(shadowContainer, TweenInfo.new(0.2, Enum.EasingStyle.Cubic), {ImageTransparency = 1}):Play()
        wait(0.2)
        mainWindow.Visible = false
        mainWindow.Size = UDim2.new(0, 450, 0, 700)  -- Reset size
        shadowContainer.ImageTransparency = 0.5  -- Reset transparency
    end)
    ...
    -- MATCHA: Horizontal tab container (50px tall)
    ...
    local function getScaleFactor()
        local currentSize = mainWindow.AbsoluteSize
    ...
    local function updateScaledElements()
        ...
        -- Update shadow size
        shadowContainer.Size = UDim2.new(0, mainWindow.AbsoluteSize.X + 40, 0, mainWindow.AbsoluteSize.Y + 40)
    end
    ...
    UserInputService.InputBegan:Connect(function(input)
        if CanChangeVisibility and input.KeyCode == key then
            ...
            if library._backgroundOverlay then
                library._backgroundOverlay.Enabled = mainWindow.Visible
            end
        end
    end)
    ...
    function TabLibrary:NewTab(title)
        ...
        -- MATCHA: Tab button styling
        tabButton.Name = title
        tabButton.Parent = tabContainer
        tabButton.BackgroundColor3 = library.theme.backgroundLight  -- #232323
        tabButton.Size = UDim2.new(0, 0, 0, 36)  -- Auto-width, 36px height
        tabButton.AutomaticSize = Enum.AutomaticSize.X  -- Auto-size based on text
        tabButton.AutoButtonColor = false
        tabButton.Font = library.currentFont
        tabButton.Text = title
        tabButton.TextColor3 = library.theme.textSecondary  -- #C3C6C6
        tabButton.TextSize = 14
        tabButton.RichText = true
        library:TrackText(tabButton)
        library:TrackElement(tabButton, "backgroundLight")

        -- MATCHA: Rounded TOP corners only (12px)
        tabCorner.CornerRadius = UDim.new(0, 12)
        tabCorner.Parent = tabButton

        -- MATCHA: Orange stroke (hidden initially)
        tabStroke.Name = "tabStroke"
        tabStroke.Parent = tabButton
        tabStroke.Color = library.theme.accent  -- #FF6A21
        tabStroke.Thickness = 2
        tabStroke.Transparency = 1  -- Hidden by default
        tabStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
        library:TrackElement(tabStroke, "accent")
        ...
        if TabLibrary.IsFirst then
            page.Visible = true
            tabButton.BackgroundColor3 = library.theme.accent  -- Orange
        ...
        tabButton.MouseButton1Click:Connect(function()
            ...
            for i,v in pairs(tabContainer:GetChildren()) do
                if v:IsA("TextButton") then
                    TweenService:Create(v, TweenTable["tab_switch"], {BackgroundColor3 = library.theme.backgroundLight}):Play()
                    TweenService:Create(v, TweenTable["tab_switch"], {TextColor3 = library.theme.textSecondary}):Play()
                    local stroke = v:FindFirstChild("tabStroke")
                    if stroke then
                        TweenService:Create(stroke, TweenTable["tab_switch"], {Transparency = 1}):Play()
                    end
                end
            end
            
            -- Set this tab as active
            TweenService:Create(tabButton, TweenTable["tab_switch"], {BackgroundColor3 = library.theme.accent}):Play()
            TweenService:Create(tabButton, TweenTable["tab_switch"], {TextColor3 = library.theme.textPrimary}):Play()
        end)
        
        -- MATCHA: Hover effects
        tabButton.MouseEnter:Connect(function()
            if TabLibrary.CurrentTab ~= title then
                TweenService:Create(tabStroke, TweenTable["tab_switch"], {Transparency = 0.5}):Play()
            end
        end)
        
        tabButton.MouseLeave:Connect(function()
            if TabLibrary.CurrentTab ~= title then
                TweenService:Create(tabStroke, TweenTable["tab_switch"], {Transparency = 1}):Play()
            end
        end)
        ...
        function Components:NewButton(text, callback)
            ...
            -- Matcha: Add orange border stroke
            local buttonStroke = Instance.new("UIStroke")
            buttonStroke.Name = "buttonStroke"
            buttonStroke.Parent = button
            buttonStroke.Color = library.theme.accent  -- #FF6A21 orange
            buttonStroke.Thickness = 1.5
            buttonStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
            library:TrackElement(buttonStroke, "accent")
        ...
        function Components:NewSlider(text, suffix, compare, compareSign, values, callback)
            ...
            sliderValue.TextColor3 = library.theme.accent  -- Matcha: Orange value text
            sliderValue.TextSize = 14.000
            sliderValue.TextXAlignment = Enum.TextXAlignment.Right
            library:TrackText(sliderValue)
            library:TrackElement(sliderValue, "accent")
